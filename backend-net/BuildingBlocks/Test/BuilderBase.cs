...
public abstract class BuilderBase<T> where T : class
{
    ...
    {
        _itemType = typeof(T);
        Item = Activator.CreateInstance(_itemType, BindingFlags.Instance | BindingFlags.NonPublic | BindingFlags.Public, null, constructorParameters, null) as T;
    }
    ...
    protected void SetProperty<TProperty>(Expression<Func<T, TProperty>> propertyFunc, TProperty value)
    {
        ...
        if (propertyInfo == null) throw new InvalidOperationException($"Expression does not match a property: {propertyFunc}");
